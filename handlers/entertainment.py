...
async def asupan(client, message):
    try:
        ...
        await message.reply_text("`Algo salió mal ;(`")
...
async def lirik(_, message):
    try:
        if len(message.command) < 2:
            await message.reply_text("**¡dale un nombre de letra también!**")
            return
        query = message.text.split(None, 1)[1]
        rep = await message.reply_text("🔎 **buscando letras ....**")
        resp = requests.get(f"https://api-tede.herokuapp.com/api/lirik?l={query}").json()
        result = f"{resp['data']}"
        await rep.edit(result)
    except Exception:
        await rep.edit("**Letra no encontrada. ** Por favor, ingrese un nombre de canción válido. !")
