...
async def pause(_, message: Message):
    try:
      ...
      await message.reply("¿Tu chat ya está conectado? ?")
      return    
    chat_id = chid
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
        callsmusic.pytgcalls.active_calls[chat_id] == "paused"
    ):
        await message.reply_text("❗ nada esta Reproduciendo!")
    else:
...
async def skip(_, message: Message):
    ...
    try:
      ...
      await message.reply("¿El chat ya está conectado??")
      return    
    chat_id = chid
    if chat_id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("❗ nada que saltar!")
    else:
    ...
    if not qeue:
        return
    await message.reply_text(f"- Omitido **{skip[0]}**\n- reproduciendo ahora**{qeue[0][0]}**")
...
async def admincache(client, message: Message):
    ...
    await message.reply_text("✅ caché de administrador actualizado!")
